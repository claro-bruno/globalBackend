...
export class UpdateInventoriesSequenceController {
    async handle(request: Request, response: Response, next: NextFunction) {
        ...
        const { created_at, fk_id_inventory, status, fk_user, seq } = request.body;
        ...
        const result = await updateInventoriesSequenceUseCase.execute({ id: +id, created_at, fk_id_inventory: +id_equipament, status, fk_user, seq });
