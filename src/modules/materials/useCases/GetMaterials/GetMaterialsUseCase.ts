...
export class GetMaterialsUseCase {
    async execute() {
        ...
        const result: any = await prisma.$queryRaw`
          ...
          (SELECT c.first_name FROM public."contractors" AS c INNER JOIN public."materialsTransactions" as it ON it.fk_id_output = c.id  WHERE c.id = mat.fk_user AND it.fk_id_output = 364) AS first_name
