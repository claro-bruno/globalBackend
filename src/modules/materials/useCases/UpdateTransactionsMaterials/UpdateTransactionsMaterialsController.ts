...
export class UpdateTransactionsMaterialsController {
    async handle(request: Request, response: Response, next: NextFunction) {
        ...
        const result = await updateTransactionsMaterialsUseCase.execute({ id: +id, fk_id_material, quantity, total_cost: +total, fk_id_input, fk_id_output, description, created_at });
