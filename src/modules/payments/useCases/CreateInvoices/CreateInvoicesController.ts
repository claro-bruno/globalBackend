...
export class CreateInvoiceController {
    async handle(request: Request, response: Response, next: NextFunction)  {
        ...
        if(date_invoice  && value && identification && id_client) {
            const result = await createInvoicesUseCase.execute({ date_invoice, value, identification, fk_id_client: +id_client });
