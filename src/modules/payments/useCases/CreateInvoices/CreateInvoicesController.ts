...
export class CreateInvoicesController {
    async handle(request: Request, response: Response, next: NextFunction) {
        ...
        const result = await createInvoicesUseCase.execute({ date_invoice, value, identification, description, fk_id_client: id_client, taxa, total_pago, date_payment: data_pagamento, method, ref, fk_id_order: id_order, quarter });
