...
export class UpdateInvoicesController {
    async handle(request: Request, response: Response, next: NextFunction)  {
        ...
        if(date_invoice && payed_for && value && identification && id_client && id) {
            const result = await updateInvoicesUseCase.execute({ date_invoice, payed_for, value, identification, fk_id_client: +id_client, id: +id });
